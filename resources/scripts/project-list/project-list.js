  ...
  for (const project of projects) {
    ...
    if (project.longDescription) {
      ...
      longDesc += project.longDescription
        .map((item) => `<li><span class="list-item-spacer">${item}</span></li>`)
        .join('\n');
      longDesc += '</ol>';
    }

    const listItem = document.createElement('li');
    const itemHTML = `<span class="list-item-spacer"><a href="${project.url}" target="_blank" rel="noopener">${project.title}</a> ${project.description}${technologies}${longDesc}</span>`;
